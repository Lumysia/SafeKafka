...
\title{SafeStream-Kafka: Real-Time Safety Analytics for IoT Workplace Video Streams\\
}
...
\item Run a per-frame YOLO classifier on each frame to label safety behavior (a detection mode is also available in code).
...
the four services communicating only through three Kafka topics; the complete
source code is available in the project repository~\cite{b14}. In
...
The as-built prototype does not yet enforce this separation. The dashboard
currently embeds its own copy of the aggregator and re-publishes alerts in
...
The per-frame YOLO classifier is one of two model families trained for
...
% 4.3 Prototype Capabilities Demonstrated by Code
\subsection{Prototype Capabilities Demonstrated by Code}

The implemented prototype demonstrates the main capabilities required for
a real-time workplace safety analytics system:

\begin{itemize}
    \item Real-time frame-event generation from files, directories,
    webcams, or RTSP sources.

    \item Kafka event transport with configurable local or Confluent Cloud
    broker settings.

    \item Per-frame YOLO classification over streamed frames (detection mode available in code).

    \item Per-camera cumulative safe and unsafe counts.

    \item Per-camera rolling-window unsafe ratios.

    \item Threshold-based warning and high-severity alerts.

    \item Live dashboard state over HTTP and WebSocket.

    \item Recent-frame display and optional detection-box overlays for
    detection-model outputs.

    \item Aggregated unsafe-behavior statistics across monitored camera
    streams.

    \item Live visualization of unsafe-ratio trends for operational safety
    monitoring.

    \item Dashboard-level operational metrics including throughput,
    alert frequency, latency estimates, and evaluation statistics.
\end{itemize}

The dashboard provides one interface for live video monitoring,
safety-event analytics, and operational metrics. Operators can monitor
individual camera streams, review safety classifications, observe alert
activity, and track system-level metrics in real time. These capabilities
cover the implementation side of the rubric: computer-vision inference,
stream processing, and interactive visualization in one Kafka-based
system.
...
% =========================================================
\section{Technical Writing Quality and Reproducibility}

The repository is reasonably reproducible for a prototype. It includes
setup instructions for macOS, Linux, and Windows, together with a local
Kafka broker through Docker Compose and configuration for Confluent
Cloud. The code is divided into producer, detector, aggregator,
dashboard, common utilities, scripts, and tests. Runtime configuration
is managed through environment-variable settings.

What works now:

\begin{itemize}
    \item A dependency list covering the core streaming, computer-vision,
    dashboard, and testing libraries.

    \item An example settings file documenting the Kafka, producer, detector,
    aggregator, and dashboard options.

    \item A Docker Compose setup that provides a local Kafka broker.

    \item A script that creates the Kafka topics automatically.

    \item Dataset-preparation and training scripts that document the
    classification-dataset preparation and model-training workflow.

    \item A test for the aggregator that validates the core aggregation and
    label-mapping behavior without requiring Kafka or YOLO.

    \item The trained per-frame classifier weights used for the offline and
    streaming results reported earlier.

    \item Saved results files that record the per-frame offline metrics and the
...
\begin{itemize}
    \item The dataset is not committed: the raw video and the prepared
    classification-tree directories are excluded from version control.

    \item The dataset-loading library required by the default hub-dataset
    preparation workflow is not currently listed in the dependency manifest.

    \item Full end-to-end tests require Kafka, a trained model, and
    sample video data. These resources are not yet included as a small
    smoke-test fixture.
\end{itemize}
...
aggregator and re-publishes alerts in parallel with the standalone aggregator;
...
Yujie Lin: Converted the existing technical report draft into the required IEEE format, refined the report layout, and created the system architecture diagram and dataset configuration table based on the project implementation.
...
\bibitem{b14} V. B. Raposo, Y. Lin, L. Zhang, Q. T. Phung, and A. Seyam, ``SafeStream-Kafka: source code,'' 2024. [Online]. Available: \url{https://github.com/Lumysia/SafeKafka}
\end{thebibliography}
